...
function [xk, dk, ak, iWk, betak, it] = solver(x, f, df, d2f, amin, amax, p, c1, c2, iW, tol, itmax, isd, icg, irc, nu)
    betak = [];
    if isd == 1
       [xk, dk, ak, iWk, it] = GM(x, f, df, amin, amax, p, c1, c2, iW, tol, itmax);
    elseif isd == 2
        [xk, dk, ak, betak, iWk, it] = CGM(x, f, df, amin, amax, p, c1, c2, iW, tol, itmax, icg, irc, nu);
    end
end